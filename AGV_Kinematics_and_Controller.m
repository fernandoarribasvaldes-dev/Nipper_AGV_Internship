...
function main_simulation
    ...
    for k = 1:N
        ...
        %% Calculate 5-second interval index (1 for t in [0,5), 2 for t in [5,10), etc.)
        idx = floor(t_curr / 5) + 1;
        idx = min(idx, length(cmd.vx)); % Protect against out-of-bounds at t = 60
        
        % Extract constant velocity commands for the current 5-second block
        Vx = cmd.vx(idx);
        Vy = cmd.vy(idx);
        W  = cmd.w(idx); 
